...
class Config:
  def __init__(self, debug=False):
    self.debug = debug
    self.filename = os.path.join(Path(os.path.split(__file__)[0]).parent,
                                 'resources', 'config.ini')
    self.config = configparser.ConfigParser()
    self.config.optionxform = lambda \
        option: option  # prevent the key value being lowercase
